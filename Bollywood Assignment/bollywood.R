...
# Store the movie name in variable named top_2_movie
	...
	Top_movies <- bollywood$Movie[order(bollywood$Tcollection,decreasing = T)]
	top_2_movie <- Top_movies[2]
	top_2_movie 
...
# Store the names of 4 movies in same sequence in movie_result vector
	...
	movie_result <- c(bollywood$Movie[which.max(bollywood$Fwcollection)],bollywood$Movie[which.max(bollywood$Wcollection)],bollywood$Movie[which.max(bollywood$Fwcollection)],bollywood$Movie[which.max(bollywood$Tcollection)])
	movie_result 
